...
namespace IsotopeFit
{
    ...
    public class LeastSquaresSystem
    {
        ...
        public void Solve2()
        {
            ...
            for (int i = 0; i < cutCoordinates.Count; i++)
            {
                ...
                else    // create the diagonal block and use NNLS
                {
                    ...
                    try
                    {
                        double[] sol = NNLS2(C, Vector<double>.Build.DenseOfArray(obs)).ToArray();
            ...
            // --------------------below here it is wrong

            // calculate the block partitioning
            //int[] blockSizes = new int[nonOverlap.Count - 1];

            //for (int i = 0; i < blockSizes.Length; i++)
            //{
            //    blockSizes[i] = nonOverlap[i + 1] - nonOverlap[i];  //TODO: this does not take into account the last block
            //}
            ...
            //    if (blockSizes[i] != 1)
            //    {
            //        int[] indexes = Enumerable.Range(nonOverlap[i] + 1, blockSizes[i] - 1).ToArray();
            //        int start = nonOverlap[i] + 1;
            ...
            //        double[] sol = NNLS(C, Vector<double>.Build.DenseOfArray(obs)).ToArray();
        ...
        public void Solve()
        {
            ...
            Solution = NNLS2(DesignMatrix, ObservationVector);
